...
router.get("/stats", (req, res) => {
  ...
  const graduated = db
    .prepare("SELECT COUNT(*) AS c FROM students WHERE status='graduated'")
    .get().c;

  const genderBreakdown = db
    .prepare(
      "SELECT COALESCE(gender,'Unspecified') AS gender, COUNT(*) AS count FROM students GROUP BY gender",
    )
    .all();

  const batchBreakdown = db
    .prepare(
      "SELECT batch_year, COUNT(*) AS count FROM students WHERE batch_year IS NOT NULL GROUP BY batch_year ORDER BY batch_year",
    )
    .all();

  const gradeDistribution = db
    .prepare(
      "SELECT COALESCE(grade,'Ungraded') AS grade, COUNT(*) AS count FROM enrollments GROUP BY grade",
    )
    .all();
  ...
  const recentStudents = db
    .prepare(
      "SELECT id, roll_no, first_name, last_name, batch_year, status, avatar_color, created_at FROM students ORDER BY id DESC LIMIT 5",
    )
    .all();
