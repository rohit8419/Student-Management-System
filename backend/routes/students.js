...
router.post("/", (req, res) => {
  const {
    roll_no, first_name, last_name, email, phone, gender,
    dob, address, guardian_name, batch_year, status, avatar_color,
  } = req.body;

  if (!roll_no || !first_name || !last_name || !email) {
    return res.status(400).json({ error: "roll_no, first_name, last_name and email are required" });
  }
  ...
  try {
    ...
    `);
    const info = stmt.run(
      roll_no, first_name, last_name, email, phone || null, gender || null,
      dob || null, address || null, guardian_name || null,
      batch_year || null, status || "active", avatar_color || "#1B2A4A"
    );
...
router.put("/:id", (req, res) => {
  ...
  try {
    db.prepare(`
      ...
      WHERE id = ?
    `).run(
      merged.roll_no, merged.first_name, merged.last_name, merged.email, merged.phone,
      merged.gender, merged.dob, merged.address, merged.guardian_name, merged.batch_year,
      merged.status, merged.avatar_color, req.params.id
    );
...
router.post("/:id/enroll", (req, res) => {
  ...
  try {
    db.prepare("INSERT INTO enrollments (student_id, course_id) VALUES (?, ?)").run(
...
router.put("/enrollments/:enrollmentId", (req, res) => {
  ...
  db.prepare("UPDATE enrollments SET grade = ? WHERE id = ?").run(grade, req.params.enrollmentId);
  res.json({ success: true });
});

// DELETE /api/students/enrollments/:enrollmentId
router.delete("/enrollments/:enrollmentId", (req, res) => {
  db.prepare("DELETE FROM enrollments WHERE id = ?").run(req.params.enrollmentId);
  res.json({ success: true });
});
